...
## Changes to arguments/new arguments
...
* `step_pca()` attempts to have more consistent signs for the scores (across data changes). If the leading element of an eigenvector is negative, the vector is negated. This only affects the sign of the scores and only for recipes prepared from this point on (i.e., existing recipes are unaffected) (#653)
