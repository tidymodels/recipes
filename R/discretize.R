...
#' 
#' \code{discretize} converts a numeric vector into a factor with bins having approximately the same number of data points (based on a training set).
#' 
#' @export
...
#' @param labels A character vector defining the factor levels that will be in the new factor (from smallest to largest). This should have length \code{cuts+1} and should not include a level for missing (see \code{keep_na} below).
#' @param prefix A single parameter value to be used as a prefix for the factor levels (e.g. \code{bin1}, \code{bin2}, ...). If the string is not a valid R name, it is coerced to one. 
...
#' @export
#' @details \code{discretize} estimates the cut points from \code{x} using percentiles. For example, if \code{cuts = 3}, the function estimates the quartiles of \code{x} and uses these as the cut points. If \code{cuts = 2}, the bins are defined as being above or below the median of \code{x}. 
#' 
#'The \code{predict} method can then be used to turn numeric vectors into factor vectors. 
#'
#'If \code{keep_na = TRUE}, a suffix of "_missing" is used as a factor level (see the examples below). 
#'
#'If \code{infs = FALSE} and a new value is greater than the largest value of \code{x}, a missing value will result. 
#'@examples 
#'data(biomass)
#' 
...
discretize.numeric <- function(x, cuts = 4, labels = NULL, prefix = "bin", 
  ...
  if(unique_vals/(cuts+1) >= min_unique) {
    ...
    if(num_breaks > length(breaks)) 
      warning("Not enough data for ", cuts, 
              " breaks. Only ",
              length(breaks), 
              " breaks were used.", sep = "")
    if(infs) {
      breaks[1] <- -Inf
    ...
    out <- list(breaks = breaks, 
                ...
                prefix = prefix,
                labels =  if(keep_na) labels <- c(missing_lab, labels) else labels,
                keep_na = keep_na)
  } else {
    out <- list(bins = 0)
...
predict.discretize <- function(object, newdata, ...) {
  if(is.matrix(newdata) | is.data.frame(newdata)) newdata <- newdata[,1] 
  object$labels <- paste0(object$prefix, object$labels)
  if(object$bins >= 1) {
    labs <- if(object$keep_na) object$labels[-1] else object$labels
    out <- cut(newdata, object$breaks, labels = labs, include.lowest = TRUE)
    if(object$keep_na) {
      out <- as.character(out)
      if(any(is.na(newdata))) out[is.na(newdata)] <- object$labels[1]
...
#' @export
print.discretize <- function(x, digits = max(3L, getOption("digits") - 3L), ...) {
  if(length(x$breaks) > 0) {
    cat("Bins:", length(x$labels))
    if(any(grepl("_missing", x$labels))) cat(" (includes missing category)")
    cat("\n")
    
    if(length(x$breaks) <= 6) {
      cat("Breaks:", paste(signif(x$breaks, digits = digits), collapse = ", "))
    }
  } else {
    if(x$bins == 0) 
      cat("Too few unique data points. No binning.") else 
...
step_discretize_new <- function(terms = NULL, role = NA, trained = FALSE, 
...
bin_wrapper <- function(x, args) {
  bin_call <- quote(discretize(x, cuts, labels, prefix, keep_na, infs, min_unique, ...))
...
learn.step_discretize <- function(x, training, info = NULL, ...) {
  ...
  step_discretize_new(terms = x$terms, role = x$role, 
...
print.step_discretize <- function(x, form_width = 30, ...) {
  cat("Discretization for ")
