...
#' @param ... One or more selector functions to choose which variables are being assigned a role. See \code{\link{selections}} for more details.
#' @param new_role A character string for a single role.
#' @return An updated recipe object.
#' @details If a variable is selected that currently has a role, the role is changed and a warning is issued.
#' @keywords datagen
...
add_role <- function(recipe, ..., new_role = "predictor") {
  ...
  if(any(!existing_var)) {
    ...
    new_vars <- tibble(variable = vars[!existing_var], role = rep(new_role, sum(!existing_var)))
    ...
    recipe$var_info$role[recipe$var_info$variable %in% vars2] <- new_role
  }
