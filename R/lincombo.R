...
#'
#' \code{step_lincomb} creates a \emph{specification} of a recipe step that will potentially remove numeric variables that have linear combinations between them.
#'
...
#' @export
#'
#' @details This step finds exact linear combinations between two or more variables and recommends which column(s) should be removed to resolve the issue. This algorithm may need to be applied multiple times (as defined by \code{max_steps}).  
#' @examples
#' data(biomass)
#' 
...
#' @seealso \code{\link{step_nzv}}\code{\link{step_corr}} \code{\link{recipe}} \code{\link{learn.recipe}} \code{\link{process.recipe}}

step_lincomb <- function(recipe,
                         ...,
                         role = NA,
                         trained = FALSE,
                         max_steps = 5,
...
step_lincomb_new <- function(terms = NULL,
                             role = NA,
                             trained = FALSE,
                             max_steps = NULL,
                             removals = NULL) {
...
print.step_lincomb <- function(x,  width = max(20, options()$width - 36), ...) {
  if(x$trained) {
    if(length(x$removals) > 0) {
    cat("Linear combination filter removed ")
    cat(format_ch_vec(x$removals, width = width))
    } else cat("Linear combination filter removed no terms")
  } else {
...
recommend_rm <- function(x, eps  = 1e-6, ...) {
  ...
  numColumns <- ncol(qr_decomp_R)          # number of columns in R
  ...
  if (is.null(numColumns) || rank == numColumns) {
    ...
    rm_list <- unlist(lapply(combos, function(x) x[1]))
...
iter_lc_rm <- function(x, max_steps = 10, verbose = FALSE) {
  ...
  for(i in 1:max_steps) {
    ...
    if(length(lcs) == 0)
      break() else {
        if(verbose) cat(" removing", length(lcs), "\n")
