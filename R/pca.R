...
#' @param threshold A fraction of the total variance that should be covered by the components. For example, \code{threshold = .75} means that \code{step_pca} should generate enough components to capture 75\% of the variability in the variables. Note: using this argument will override and resent any value given to \code{num}.   
#' @param options A list of options to the default method for \code{\link[stats]{prcomp}}. Argument defaults are set to \code{retx = FALSE}, \code{center = FALSE}, \code{scale. = FALSE}, and \code{tol = NULL}. \bold{Note} that the argument \code{x} should not be passed here (or at all).
#' @param res The \code{\link[stats]{prcomp.default}} object is stored here once this preprocessing step has be trained by \code{\link{learn.recipe}}.
...
#'
#' It is advisable to standardized the variables prior to running PCA. Here, each variable will be centered and scaled prior to the PCA calculation. This can be changed using the \code{options} argument or by using \code{\link{step_center}} and \code{\link{step_scale}}. 
#'
...
#' 
#' Alternatively, \code{threshold} can be used to determine the number of components that are required to capture a specified fraction of the total variance in the variables. 
#'
...
learn.step_pca <- function(x, training, info = NULL, ...) {
  ...
  prc_call <- expr(prcomp(retx = FALSE, center = FALSE, scale. = FALSE, tol = NULL))
  ...
  if(!is.na(x$threshold)) {
    ...
    num_comp <- which.max(cumsum(prc_obj$sdev^2/total_var) >= x$threshold)
...
print.step_pca <- function(x, width = max(20, options()$width - 29), ...) {
