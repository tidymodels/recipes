...
process.step_ns <- function(object, newdata, ...) {
  ...
  ns_values <- matrix(NA, nrow = nrow(newdata), ncol = sum(new_cols))
  colnames(ns_values) <- rep("", sum(new_cols))
  strt <- 1
  for(i in names(object$objects)) {
    cols <- (strt):(strt+new_cols[i]-1)
    orig_var <- attr(object$objects[[i]], "var")
    ns_values[, cols] <- predict(object$objects[[i]], getElement(newdata, i))
    new_names <- paste(orig_var, "ns", names0(new_cols[i], ""), sep = "_")
    colnames(ns_values)[cols] <- new_names
    strt <- max(cols)+1
    newdata[, orig_var] <- NULL
  }
  ns_values <- as_tibble(ns_values)
  newdata <- cbind(newdata, ns_values)
