...
step_date <- function(recipe,
                      ...
                      ordinal = FALSE,
                      variables = NULL) {
  feat <- c("year", "doy", "week", "decimal", "semester", "quarter", "dow", "month")
...
step_date_new <- function(terms = NULL,
                          role = "predictor",
                          trained = FALSE,
                          features = features,
                          abbr = abbr,
...
learn.step_date <- function(x, training, info = NULL, ...) {
  ...
  if(any(date_data$type != "date"))
    stop("All variables for `step_date` should be either `Date` or `POSIXct` classes.")
...
get_date_features <- function(dt, feats, abbr = TRUE, label = TRUE, ord = FALSE) {
  ## pre-allocate values
  ...
  if("dow" %in% feats) {
    res[, grepl("dow$", names(res))] <- wday(dt, abbr = abbr, label = label)
    if(!ord & label == TRUE) res[, grepl("dow$", names(res))]  <-
        ord2fac(res, grep("dow$", names(res), value = TRUE))
  }
  if("month" %in% feats) {
    res[, grepl("month$", names(res))] <- month(dt, abbr = abbr, label = label)
    if(!ord & label == TRUE) res[, grepl("month$", names(res))]  <-
...
print.step_date <- function(x, width = max(20, options()$width - 29), ...) {
