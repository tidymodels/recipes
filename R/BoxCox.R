...
#'
#' @references Sakia, R. M. (1992). The Box-Cox transformation technique: A review. \emph{The Statistician}, 169-178..
#' @examples
#'
#' rec <- recipe(~ ., data = as.data.frame(state.x77))
#'
#' bc_trans <- step_BoxCox(rec, terms = ~ is_numeric())
#'
...
print.step_BoxCox <- function(x, width = max(20, options()$width - 35), ...) {
