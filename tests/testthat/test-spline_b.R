...
test_that("correct basis functions", {
  ...
  carbon_ns_tr_exp <- splines2::bSpline(biomass_tr$carbon, df = 5)
  hydrogen_ns_tr_exp <- splines2::bSpline(biomass_tr$hydrogen, df = 5)
...
test_that("check_options() is used", {
  ...
  expect_snapshot(
    error = TRUE,
    recipe(~mpg, data = mtcars) |>
      step_spline_b(mpg, options = TRUE) |>
      prep()
  )
})
...
test_that("bad args", {
  ...
  expect_snapshot(
    recipe(mpg ~ ., data = mtcars) |>
      step_spline_b(disp, degree = -1) |>
      prep(),
    error = TRUE
  )
  expect_snapshot(
    recipe(mpg ~ ., data = mtcars) |>
      step_spline_b(disp, deg_free = "a") |>
      prep(),
    error = TRUE
  )
  expect_snapshot(
    recipe(mpg ~ ., data = mtcars) |>
      step_spline_b(disp, complete_set = 1) |>
      prep(),
    error = TRUE
  )
})
