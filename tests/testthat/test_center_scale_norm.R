...
test_that("scale - works with integer64", {
...
test_that("center - works with integer64", {
  df <- data.frame(x = bit64::as.integer64(1:5))

  rec <- recipe(~ x, df)
  rec <- step_center(rec, x)
  ...
  expect <- tibble::tibble(x = as.double(df$x) - mean(as.double(df$x)))
