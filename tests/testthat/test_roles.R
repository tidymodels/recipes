...
test_that("default method", {
  ...
  exp_res <- tibble(
    ...
    type = rep(c("unordered", "double"), c(2, 6)),
    role = NA_character_,
    source = "original"
  )
...
test_that("new role with specified type", {
  ...
  exp_res <- tibble(
    ...
    type = c("unordered", "new type", "unordered", rep("double", 6)),
...
test_that("add new role when two already exist with different types", {
  ...
  exp_res <- tibble(
    ...
    type = c("unordered", "new type", "unordered", "unordered", rep("double", 6)),
...
test_that("existing role is skipped", {
  ...
  exp_res <- tibble(
    variable = c("sample", colnames(biomass)),
    type = rep(c("unordered", "double"), c(3, 6)),
    role = rep(c("blah", "some other role", NA), c(1, 1, 7)),
    source = "original"
  )
  expect_equal(summary(rec, TRUE), exp_res)
...
test_that("existing role is skipped, but new one is added", {
  ...
  exp_res <- tibble(
    ...
    type = rep(c("unordered", "double"), c(4, 6)),
...
test_that("New type for an existing role can be added", {
  ...
  exp_res <- tibble(
    variable = c("sample", colnames(biomass)),
    type = c(c("unordered", "text", "unordered"), rep("double", 6)),
