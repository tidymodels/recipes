...
test_that("correct monotone functions", {
  ...
  with_ns <- rec %>%
    step_spline_monotone(carbon, hydrogen, deg_free = 5)
...
test_that("errors if degree > deg_free (#1170)", {
  ...
  expect_no_error(
    recipe(~., data = mtcars) %>%
      step_spline_monotone(mpg, degree = 2, deg_free = 3, complete_set = TRUE) %>%
      prep()
  )
  ...
  expect_snapshot(
    error = TRUE,
    recipe(~., data = mtcars) %>%
      step_spline_monotone(mpg, degree = 3, deg_free = 3, complete_set = TRUE) %>%
      prep()
  )
  ...
  expect_snapshot(
    error = TRUE,
    recipe(~., data = mtcars) %>%
      step_spline_monotone(mpg, degree = 4, deg_free = 3, complete_set = FALSE) %>%
      prep()
  )
})
...
test_that("empty selection prep/bake is a no-op", {
  ...
  rec2 <- step_spline_monotone(rec1)
