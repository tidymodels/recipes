...
test_that("correct convex functions", {
  ...
  carbon_ns_tr_exp <- splines2::cSpline(biomass_tr$carbon, df = 5)
  hydrogen_ns_tr_exp <- splines2::cSpline(biomass_tr$hydrogen, df = 5)
...
test_that("check_options() is used", {
  ...
  expect_snapshot(
    error = TRUE,
    recipe(~mpg, data = mtcars) |>
      step_spline_convex(mpg, options = TRUE) |>
      prep()
  )
})
