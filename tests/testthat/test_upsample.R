...
scat2 <- scat[-(1:45),]
scat2$Species[seq(6, 40, by = 5)] <- NA
scat2$Species2 <- sample(scat2$Species)
scat2$Species3 <- as.character(sample(scat2$Species))

rec <- recipe( ~ ., data = scat2)
...
test_that('basic usage', {
  ...
  rec1_p <- prep(rec1, training = scat2)
  ...
  te_xtab <- table(bake(rec1_p, new_data = scat2)$Species, useNA = "always")
  og_xtab <- table(scat2$Species, useNA = "always")

  expect_equal(max(tr_xtab), max(og_xtab))
  expect_equal(sum(is.na(juice(rec1_p)$Species)), max(og_xtab))
  expect_equal(te_xtab, og_xtab)

  expect_warning(prep(rec1, training = scat2), NA)
})
...
test_that('ratio value', {
  ...
  rec2_p <- prep(rec2, training = scat2)

  tr_xtab <- table(juice(rec2_p)$Species, useNA = "always")
  te_xtab <- table(bake(rec2_p, new_data = scat2)$Species, useNA = "always")
  og_xtab <- table(scat2$Species, useNA = "always")

  expect_equal(min(tr_xtab), 7)
  expect_equal(sum(is.na(juice(rec2_p)$Species)),
               sum(is.na(scat2$Species)))
  expect_equal(te_xtab, og_xtab)
})
...
test_that('no skipping', {
  ...
  rec3_p <- prep(rec3, training = scat2)

  tr_xtab <- table(juice(rec3_p)$Species, useNA = "always")
  te_xtab <- table(bake(rec3_p, new_data = scat2)$Species, useNA = "always")
  og_xtab <- table(scat2$Species, useNA = "always")
...
test_that('printing', {
  ...
  expect_output(prep(rec4, training = scat2, verbose = TRUE))
})
