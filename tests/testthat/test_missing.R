...
tst <- function(...) {
  cols <- quos(...)
  recipe(set_with_na) %>% check_noNA(!!!cols) %>%
    prep() %>% bake(set_with_na)
}

test_that("check_noNA passes silently when no NA", {
  no_na_rp <- recipe(mtcars) %>%
    check_noNA(all_numeric()) %>%
    prep()
  expect_error(bake(no_na_rp, mtcars), NA)
  expect_equal(bake(no_na_rp, mtcars), mtcars)
})

test_that("check_noNA throws error on all types", {
  expect_error(tst(a),
...
test_that("check_noNA works on multiple columns simultaneously" ,{
...
test_that("check_noNA on a new set", {
  no_na <- data_frame(a = 1:3)
  na    <- data_frame(a = c(1, NA))
  rp    <- recipe(no_na) %>% check_noNA(a) %>% prep(no_na)
  expect_error(bake(rp, na),
               "The following columns contain missing values: `a`.")
})

test_that("tidy mthod for check_noNA", {
  set_with_na
})
