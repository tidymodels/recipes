...
test_that("errors if degree > deg_free (#1170)", {
  ...
  expect_no_error(
    recipe(~., data = mtcars) %>%
      step_spline_nonnegative(mpg, degree = 2, deg_free = 3, complete_set = TRUE) %>%
      prep()
  )

  expect_no_error(
    recipe(~., data = mtcars) %>%
      step_spline_nonnegative(mpg, degree = 3, deg_free = 3, complete_set = FALSE) %>%
      prep()
  )
  ...
  expect_snapshot(
    error = TRUE,
    recipe(~., data = mtcars) %>%
      step_spline_nonnegative(mpg, degree = 3, deg_free = 3, complete_set = TRUE) %>%
      prep()
  )
  ...
  expect_snapshot(
    error = TRUE,
    recipe(~., data = mtcars) %>%
      step_spline_nonnegative(mpg, degree = 4, deg_free = 3, complete_set = FALSE) %>%
      prep()
  )
})
