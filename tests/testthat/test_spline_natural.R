...
test_that("correct basis functions", {
  ...
  carbon_ns_tr_exp <- naturalSpline(biomass_tr$carbon, df = 5)
  hydrogen_ns_tr_exp <- naturalSpline(biomass_tr$hydrogen, df = 5)
...
test_that("printing", {
  ...
  with_ns <- rec %>% step_spline_natural(carbon, hydrogen)
