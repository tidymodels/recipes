...
[`update_role()`](https://recipes.tidymodels.org/reference/roles.md) for
general information about roles). Role requirements can only be altered
for roles that exist in the *original* data supplied to
[`recipe()`](https://recipes.tidymodels.org/reference/recipe.md), they
are not applied to columns computed by steps.
...
[`prep()`](https://recipes.tidymodels.org/reference/prep.md) time.
...
- bake:
  ...
  should a check be done to ensure that all columns of this role that
  were supplied to
  [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) also
  be present in the `new_data` supplied to
