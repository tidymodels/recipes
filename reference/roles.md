...
[`recipe()`](https://recipes.tidymodels.org/reference/recipe.md). See
the `role` argument in some step functions to update roles for columns
created by steps.
...
to [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md)) and
won't be used in model fitting or analysis, but will still be retained
in the data set.
...
[`recipe()`](https://recipes.tidymodels.org/reference/recipe.md). The
reason for this is because recipes assumes that all non-standard roles
are required at
[`bake()`](https://recipes.tidymodels.org/reference/bake.md) time (or
[`predict()`](https://rdrr.io/r/stats/predict.html) time, if you are
using a workflow). Since you didn't use `sample` in any steps of the
recipe, you might think that you don't need to pass it to
[`bake()`](https://recipes.tidymodels.org/reference/bake.md), but this
isn't true because recipes doesn't know that you didn't use it:
...
calling
[`recipe()`](https://recipes.tidymodels.org/reference/recipe.md). In
general, predictors and non-standard roles that are supplied to
[`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) should
be present at both
[`prep()`](https://recipes.tidymodels.org/reference/prep.md) and
[`bake()`](https://recipes.tidymodels.org/reference/bake.md) time.
...
[`bake()`](https://recipes.tidymodels.org/reference/bake.md) time. You
