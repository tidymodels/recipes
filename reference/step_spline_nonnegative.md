...
`step_spline_nonnegative()` creates a *specification* of a recipe step
that creates non-negative spline features.
...
non-negative and have interesting statistical properties (such as
integrating to one). A zero-degree M-spline generates box/step functions
while a first degree basis function is triangular.
...
Curry, H.B., Schoenberg, I.J. (1988). On Polya Frequency Functions IV:
The Fundamental Spline Functions and their Limits. In: de Boor, C. (eds)
I. J. Schoenberg Selected Papers. Contemporary Mathematicians.
Birkhäuser, Boston, MA

Ramsay, J. O. "Monotone Regression Splines in Action." Statistical
Science, vol. 3, no. 4, 1988, pp. 425–41
