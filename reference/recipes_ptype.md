...
At [`prep()`](https://recipes.tidymodels.org/reference/prep.md) time,
when `stage = "prep"`, the ptype is the data passed to
[`recipe()`](https://recipes.tidymodels.org/reference/recipe.md). The
following code chunk represents a possible recipe scenario.
...
[`prep()`](https://recipes.tidymodels.org/reference/prep.md) to verify
that `data_training` has the right columns with the right types.
...
At [`bake()`](https://recipes.tidymodels.org/reference/bake.md) time,
when `stage = "bake"`, the ptype represents the data that are required
for [`bake()`](https://recipes.tidymodels.org/reference/bake.md) to run.
