...
* *transformations* of data (replacement, uni- or multivariate): this includes logging, Box-Cox, arc-sin, spatial sign or other transformations of data. 
...
names(segmentationData)[1:2] <- LETTERS[1:2]
raw_train <- subset(segmentationData, Case == "Train")
raw_test  <- subset(segmentationData, Case == "Test")

raw_train$Case <- NULL
raw_test$Case  <- NULL
```
There are two data sets: `raw_train` with `r nrow(raw_train)` data points and `raw_test` with `r nrow(raw_test)` samples.
