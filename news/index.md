...
- [`bake()`](https://recipes.tidymodels.org/reference/bake.md) gained a
  `stop_at` argument, which takes a step number or step id and applies
  only the steps up to and including that step. This is useful for
  diagnostics such as visualizing the effect of each step.
...
- [`prep()`](https://recipes.tidymodels.org/reference/prep.md) is now
  faster on recipes with many columns or many steps.
  ([\#1543](https://github.com/tidymodels/recipes/issues/1543))
...
- New
  [`recipes_map_cols()`](https://recipes.tidymodels.org/reference/recipes_map_cols.md)
  helper for use in
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) methods
  of steps that transform columns in place. It applies a function to the
  selected columns and assigns the results in bulk, avoiding the
  quadratic cost of assigning one column at a time.
  ([\#1543](https://github.com/tidymodels/recipes/issues/1543))
...
- Fixed bug in
  ...
  where [`bake()`](https://recipes.tidymodels.org/reference/bake.md)
...
- The `strings_as_factors` argument of
  [`prep.recipe()`](https://recipes.tidymodels.org/reference/prep.md)
  has been soft-deprecated in favor of `recipe(strings_as_factors)`. If
  both are provided, the value in
  [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) takes
  precedence. This allows control of recipe behavior within a workflow,
...
- Fixed bug where
  [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) would
  error on sf objects.
  ([\#1393](https://github.com/tidymodels/recipes/issues/1393))

- [`step_cut()`](https://recipes.tidymodels.org/reference/step_cut.md)
  not longer errors on NA values in
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md).
...
- Make it so
  [`recipe.formula()`](https://recipes.tidymodels.org/reference/recipe.md)
...
- Fixed bugs in
  ...
  in [`bake()`](https://recipes.tidymodels.org/reference/bake.md) with
  zero-row data.
...
- Added developer function
  [`check_options()`](https://recipes.tidymodels.org/reference/check_options.md).
  ([\#1269](https://github.com/tidymodels/recipes/issues/1269))

- Officially deprecate
  [`printer()`](https://recipes.tidymodels.org/reference/recipes-internal.md)
  in favor of
  [`print_step()`](https://recipes.tidymodels.org/reference/recipes-internal.md).
...
- [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md),
  [`prep()`](https://recipes.tidymodels.org/reference/prep.md), and
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) now work
  with sparse tibbles.
...
- [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) can
  now take data.frames with list-columns or sf data.frames as input to
  `data`. ([\#1283](https://github.com/tidymodels/recipes/issues/1283))

- [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) will
  now show better error when columns are misspelled in formula
  ([\#1283](https://github.com/tidymodels/recipes/issues/1283)).

- [`add_role()`](https://recipes.tidymodels.org/reference/roles.md) now
  errors if a column would simultaneously have roles `"outcome"` and
  `"predictor"`.
  ([\#935](https://github.com/tidymodels/recipes/issues/935))

- [`prep()`](https://recipes.tidymodels.org/reference/prep.md) will now
  error if the ptype of the data doesn’t match which was used to define
  the recipe.
...
- [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) no
  longer crashes when given long formula expression
  ([\#1283](https://github.com/tidymodels/recipes/issues/1283)).

- Fixed bug in
  [`step_ns()`](https://recipes.tidymodels.org/reference/step_ns.md) and
  [`step_bs()`](https://recipes.tidymodels.org/reference/step_bs.md)
...
- Developer helper function
  ...
  [`prep()`](https://recipes.tidymodels.org/reference/prep.md) and
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) for a
  given recipe object.
...
- Minor speed-up and reduced memory consumption for
  ...
  in the [`bake()`](https://recipes.tidymodels.org/reference/bake.md)
  stage by reducing unused multiplications
...
- Document that
  ...
  [`add_role()`](https://recipes.tidymodels.org/reference/roles.md) and
...
- [`prep()`](https://recipes.tidymodels.org/reference/prep.md) now
...
- [`step_bs()`](https://recipes.tidymodels.org/reference/step_bs.md) and
  [`step_ns()`](https://recipes.tidymodels.org/reference/step_ns.md)
...
- Added developer function
  [`remove_original_cols()`](https://recipes.tidymodels.org/reference/remove_original_cols.md)
  to help remove original columns that are no longer needed.
  ([\#1149](https://github.com/tidymodels/recipes/issues/1149))

- Added developer function
  [`recipes_remove_cols()`](https://recipes.tidymodels.org/reference/recipes_remove_cols.md)
...
- Errors in [`prep()`](https://recipes.tidymodels.org/reference/prep.md)
  and [`bake()`](https://recipes.tidymodels.org/reference/bake.md) will
  now indicate which step caused the error.
  ([\#420](https://github.com/tidymodels/recipes/issues/420))

- Developer focused
  [`check_type()`](https://recipes.tidymodels.org/reference/check_type.md)
...
- [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) now
  error more informatively when `data` is missing.
...
- recipes now checks that all columns in the `data` supplied to
  [`recipe()`](https://recipes.tidymodels.org/reference/recipe.md) are
  also present in the `new_data` supplied to
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md). An
  exception is made for columns with roles of either `"outcome"` or
  `"case_weights"`, which are typically not required at
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) time. The
  new
  ...
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) time if
  you need to opt out of this check
...
- The [`summary()`](https://rdrr.io/r/base/summary.html) method for
  ...
  are required when
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) is used.
...
- Fixed a bug where setting `fresh = TRUE` in
  [`prep()`](https://recipes.tidymodels.org/reference/prep.md) wouldn’t
  result in re-prepping the recipe.
...
- [`step_normalize()`](https://recipes.tidymodels.org/reference/step_normalize.md)
  ...
  use [`step_zv()`](https://recipes.tidymodels.org/reference/step_zv.md)
...
- [`prep()`](https://recipes.tidymodels.org/reference/prep.md) and
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) checks
  and errors if output of `bake.bake_*()` isn’t a tibble.
...
- [`step_zv()`](https://recipes.tidymodels.org/reference/step_zv.md) now
  has a `group` argument. The same filter is applied but looks for
...
- [`bake()`](https://recipes.tidymodels.org/reference/bake.md) will now
  error if `new_data` doesn’t contain all the required columns.
...
- Escalate deprecation for old versions of imputation steps (such as
  [`step_bagimpute()`](https://recipes.tidymodels.org/reference/step_bagimpute.md))
...
- New
  ...
  [`terms_select()`](https://recipes.tidymodels.org/reference/terms_select.md)
...
- Preserve multiple roles in `last_term_info` so
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) can
...
- Changed the names of all imputation steps, for example, from
  [`step_knnimpute()`](https://recipes.tidymodels.org/reference/step_knnimpute.md)
  or
  [`step_medianimpute()`](https://recipes.tidymodels.org/reference/step_medianimpute.md)
  (old) to
  [`step_impute_knn()`](https://recipes.tidymodels.org/reference/step_impute_knn.md)
  or
  [`step_impute_median()`](https://recipes.tidymodels.org/reference/step_impute_median.md)
...
- Performance improvements for
  [`step_bs()`](https://recipes.tidymodels.org/reference/step_bs.md) and
  [`step_ns()`](https://recipes.tidymodels.org/reference/step_ns.md).
  The [`prep()`](https://recipes.tidymodels.org/reference/prep.md) step
  no longer evaluates the basis functions on the training set and the
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) steps
  only evaluates the basis functions once for each unique input value
  ([\#574](https://github.com/tidymodels/recipes/issues/574))
...
- [`prep()`](https://recipes.tidymodels.org/reference/prep.md) gained an
  option to print a summary of which columns were added and/or removed
  during execution.

- To reduce confusion between
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) and
  [`juice()`](https://recipes.tidymodels.org/reference/juice.md), the
  latter is superseded in favor of using
  ...
  [`juice()`](https://recipes.tidymodels.org/reference/juice.md) will
  remain in the package (and used internally) but most communication and
  training will use `bake(object, new_data = NULL)`.
...
- [`step_pls()`](https://recipes.tidymodels.org/reference/step_pls.md)
  ...
  [`juice()`](https://recipes.tidymodels.org/reference/juice.md) and
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md). With the
  current version, the categorical outcomes can be used but now
...
- Improved error messaging, for example when a user tries to
  [`prep()`](https://recipes.tidymodels.org/reference/prep.md) a
  tuneable recipe.
...
- [`step_zv()`](https://recipes.tidymodels.org/reference/step_zv.md) now
  handles `NA` values so that variables with zero variance *plus* are
  removed.
...
- renamed `yj_trans()` to
  [`yj_transform()`](https://recipes.tidymodels.org/reference/recipes-internal.md)
  to avoid conflicts.
...
- Using a minus in the formula to
  [`recipes()`](https://recipes.tidymodels.org/reference/recipes.md) is
  no longer allowed (it didn’t remove variables anyway).
  [`step_rm()`](https://recipes.tidymodels.org/reference/step_rm.md) or
  [`update_role()`](https://recipes.tidymodels.org/reference/roles.md)
  can be used instead.
...
- When using a selector that returns no columns,
  [`juice()`](https://recipes.tidymodels.org/reference/juice.md) and
  [`bake()`](https://recipes.tidymodels.org/reference/bake.md) will now
  return a tibble with as many rows as the original template data or the
  `new_data` respectively. This is more consistent with how selectors
  work in dplyr
...
- [`bake()`](https://recipes.tidymodels.org/reference/bake.md) now
...
- [`format_ch_vec()`](https://recipes.tidymodels.org/reference/format_ch_vec.md)
...
- [`step_other()`](https://recipes.tidymodels.org/reference/step_other.md)’s
...
- [`add_role()`](https://recipes.tidymodels.org/reference/roles.md) can
  now only add *new* additional roles. To alter existing roles, use
  [`update_role()`](https://recipes.tidymodels.org/reference/roles.md).
...
- `step_interact` now allows for selectors
  (e.g. [`all_predictors()`](https://recipes.tidymodels.org/reference/has_role.md)
