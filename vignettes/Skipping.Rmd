...
When steps are created in a recipe, they can be applied to data (i.e. `bake`ed) at two distinct times:
...
1. After the recipe has been prepared, `bake` can be used with any data set to apply the preprocessing to a data set. 
...
As of version recipes 0.1.2, each step has an optional logical argument called `skip`. In almost every case, the default is `TRUE`. When using this options:
