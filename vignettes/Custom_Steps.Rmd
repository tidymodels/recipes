...
knitr::opts_chunk$set(message = FALSE, digits = 3)
```
...
The user-exposed function `step_percentile` is just a simple wrapper around an internal function called `add_step`. This function takes the same arguments as your function and simply adds it to a new recipe. 


```{r initial_def}
step_percentile <- function(recipe, terms, role = NA, 
...
It might seen odd that any price of information that might be used later (e.g. `ref_dist`) is also an argument, but we recommend doing so.  
...
You will need to create a new `learn` method for your step's class. To do this, three arguments that the method should have:
 ...
 * `info` will also be a tibble that has information on the current set of data available. This information is updated as each step is processed by its specific `learn` method so it may not have the variables from the original data. The columns in this tibble are `variable` (the variable name), `type` (currently either "numeric" or "nominal"), `role` (defining the variable's role), and `source` (either "original" or "derived" depending on where it originated).

You can define other options. 

The first thing that you might want to do in the `learn` function is to translate the specification listed in the `terms` argument to column names in the current data. There is an internal function called `parse_terms_formula` that can be used to obtain this. 

```{r learn_1, eval = FALSE}
learn.step_percentile <- function(x, training, info = NULL, ...) {
  col_names <- recipes:::parse_terms_formula(x$terms, info = info) 
}
```
...
Remember that the `learn` function does not _apply_ the step to the data; it only estimates any required values such as `ref_dist`. We will need to create a new method for our `step_percentile` class. The minimum arguments for this are

```r
function(object, newdata, ...)
```

where `object` is the updated step function that has been through the corresponding `learn` code and `newdata` is a tibble of data to be processed. 

Here is the code to convert the new data to percentiles. Two initial help functions handle the two cases (approximation or not). We always return a tibble as the output. 
...
process.step_percentile <- function(object, newdata, ...) {
  require(tibble)
...
rec_obj <- rec_obj %>%
  step_percentile(~ is_predictor(), approx = TRUE) 

rec_obj <- learn(rec_obj, training = biomass_tr)

percentiles <- process(rec_obj, biomass_te)
percentiles
```
