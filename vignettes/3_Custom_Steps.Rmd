...
## process but do not evaluate the variable selectors with
## the `dots_quos` function in `rlang`
  terms <- rlang::dots_quos(...) 
  if(length(terms) == 0)
...
The first thing that you might want to do in the `learn` function is to translate the specification listed in the `terms` argument to column names in the current data. There is an internal function called `select_terms` that can be used to obtain this. 

```{r learn_1, eval = FALSE}
learn.step_percentile <- function(x, training, info = NULL, ...) {
  col_names <- select_terms(info = info, args = x$terms) 
}
```
