...
Notice that integer variables have roles `"integer"` and `"numeric"`, and the factor variable have roles `"factor"`, `"unordered"`, `"nominal"`. This allows for some neat selections where the selector `all_numeric()` select double and integer variables, and more specific selectors such as `all_integer()` only selects integer variables. Full hierachy of types can be seen in `?has_role`.
